...
Page({
  ...
  savePrize: function () {
   ...
   str =  str.substring(0, str.lastIndexOf('</br>'));
    ...
    if (that.data.action == 'update') {
      wx.request({
        url: "https://www.api.yyhelper.icu/1.0/prizes/" + that.data.id,
        method: 'PUT',
        data: {
          info: str
        },
  ...
  onLoad: function (options) {
    ...
    if (options.id) {
      wx.request({
        url: 'https://www.api.yyhelper.icu/1.0/prizes/' + options.id,
        method: 'GET',
        header: {
        ...
        success: function (data) {
          if(data.data.statuscode == 200){
            ...
            //循环赋值
            for (let i = 0; i < 10; i++) {
              if (data.data.prize.info[i] != undefined) {
                //赋值
                INFO[i] = {
                  idx: i,
                  value: data.data.prize.info[i],
                  hiddena: false
                }
              } else {
                if (data.data.prize.info[i - 1] != undefined) {
                  IDX = i;
                }
                INFO[i] = {
                  idx: i,
